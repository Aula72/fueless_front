...
function MaxLengthCounter({
  ...
  const {
    isFocused
  } = hooks_UNSAFE_useFormFieldContext.useFormFieldContext(); // We don't want mousedown on this component to do anything, so prevent the default behavior.

  const killEvent = hooks.useCallback(event => {
    event.preventDefault();
  }, []);
  const textClasses = classNames.classNames([!isFocused && remainingTextStyles.hide]);
  const {
    pressProps
...
const TextArea = compat.forwardRef(({ ariaDescribedBy, assistiveText, autoComplete = 'off', autoFocus = false, helpSourceLink, helpSourceText, isDisabled: propIsDisabled, isReadonly: propIsReadonly, isRequired = false, isRequiredShown, label, labelEdge: propLabelEdge, labelStartWidth: propLabelStartWidth, maxLength, maxLengthCounter = 'remaining', maxLengthUnit, messages, placeholder, resize, role, rows, textAlign: propTextAlign, userAssistanceDensity: propUserAssistanceDensity, value, variant = 'default', onInput, onCommit }, ref) => {
...
/*  */
//# sourceMappingURL=TextArea-5eafa6db.js.map
