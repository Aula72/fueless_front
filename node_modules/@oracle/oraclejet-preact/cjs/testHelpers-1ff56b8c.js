/* @oracle/oraclejet-preact: 14.0.4 */
'use strict';

var preact = require('@testing-library/preact');

/**
 * List test Adapters to abstract common unit test actions away from the implementation details.
 */
const mouseItemSelection = (element, options) => {
    preact.fireEvent.click(element, options);
};
const mouseCheckboxSelection = (element, options) => {
    const checkbox = element.querySelector('[type=checkbox]');
    if (checkbox) {
        preact.fireEvent.click(checkbox, options);
    }
    else {
        throw new Error('checkbox not found!');
    }
};
const getRoot = async (list, role = 'grid') => {
    return list.findByRole(role);
};
const getItems = async (list, role = 'row') => {
    return list.findAllByRole(role);
};
const getContents = async (list, role = 'gridcell') => {
    return list.findAllByRole(role);
};

exports.getContents = getContents;
exports.getItems = getItems;
exports.getRoot = getRoot;
exports.mouseCheckboxSelection = mouseCheckboxSelection;
exports.mouseItemSelection = mouseItemSelection;
/*  */
//# sourceMappingURL=testHelpers-1ff56b8c.js.map
