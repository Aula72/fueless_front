...
const useTooltipControlled = ({ text, isOpen = false, variant = 'tooltip', position = 'bottom', isDisabled = false, anchor = { x: 'element', y: 'element' }, offset = { mainAxis: 0, crossAxis: 0 }, onToggle }) => {
    ...
    const { hoverProps, isHover } = hooks_UNSAFE_useHover.useHover({ isDisabled });
    ...
    */
    ...
    let actionableProps;
    if (disabled) {
        actionableProps = utils_UNSAFE_mergeProps.mergeProps(focusProps, focusInitProps);
    }
    else {
        const anchorInitProps = Object.assign({ 'aria-describedby': uniqueID, onMouseEnter: (event) => {
                if (event.eventPhase === Event.AT_TARGET ||
                    (event.eventPhase === Event.BUBBLING_PHASE && isDatatip)) {
                    targetRef.current = event.target;
                }
        ...
        actionableProps = utils_UNSAFE_mergeProps.mergeProps(hoverProps, focusProps, touchProps, focusInitProps, anchorInitProps);
    }
