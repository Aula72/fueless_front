...
import { OjMenu, ojMenu, OjSwipeActions, ojSwipeActions } from '../elements';
...
class SlotProxyImpl implements SlotProxy {
  ...
  private _parent: WebElement;
  private _touchSupportCalled: boolean;
  ...
  private async scrollIntoView(): Promise<void> {
    if (this._touchSupported) {
      const touchActions = new TouchSequence(this.getDriver());
      const rootLocation = await this._root.getLocation();
      return touchActions
        .tapAndHold({ x: 0, y: 0 })
        .move(rootLocation)
        .release(rootLocation)
        .perform();
    } else {
  ...
  /*
   * Finds out if touch screen is supported on the device the tests are running on and and cache's results.
   */
  private async isTouchScreenSupported(): Promise<boolean> {
    if (!this._touchSupportCalled) {
      this._touchSupported = await this.getDriver().executeAsyncScript<boolean>(`
   ...
   */
  private async locateMenu(): Promise<OjMenu> {
   ...
   */
  private async getOpenMenuId(): Promise<string> {
