 ...
 * @private
 */
let currentInstance: { configName: string; driver: WebDriver } = {
  configName: null,
  driver: null
};
...
function createDriver(configName: string) {
  ...
  return quitPromise.then(async () => {
    ...
    // selenium-webdriver^3.60 has setTimeouts(),
    // but @types/selenium-webdriver^3.0.8 (latest) has timeouts(). Need to cast
    // here to avoid compilation failure.
