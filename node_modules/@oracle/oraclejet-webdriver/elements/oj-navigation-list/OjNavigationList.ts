...
Use the ExpandAllKeySet class to expand all items.
  ...
  async changeSelectionByIndex(index: number | Array<number>) {
    await this.whenReady();
    let indexArray: Array<number>, parent: WebElement;
...
Use the ExpandAllKeySet class to expand all items.
   ...
   */
  async getItemByIndex(indexPath: Array<number>): Promise<WebElement> {
    let item: WebElement;
    for (let i = 0; i < indexPath.length; i++) {
      item = item
  ...
  private async _getItemByIndex(index: number, parent?: WebElement): Promise<WebElement> {
    let item;
    try {
      if (!parent)
  ...
  private async _isCollapsed(item: WebElement) {
