...
export class OjDataGrid extends OjDataGridBase {
   ...
   */
  async getCellContents<T>(itemLocator: { key: cellSubId }): Promise<WebElement> {
    ...
    let cellProperty = await this.getCell();
    let cellMatch = null;
  ...
  async findCell<T>(itemLocator: cellSubId): Promise<SlotProxy> {
    ...
    datagridCell = await this.getDriver().executeAsyncScript<WebElement>(
      `
   ...
   */
  async getHeaderContents(subId: headerSubId, by: By): Promise<Array<WebElement> | null> {
  ...
  async findHeader<T>(itemLocator: headerSubId): Promise<SlotProxy> {
    await this.whenReady();
    ...
    let header = null;
    let headerCell = null;
    header = await this.getDriver().executeAsyncScript<WebElement>(
      `
      var element = arguments[0];
   ...
   */
  async getLabelContents(subId: labelSubId, by: By): Promise<Array<WebElement> | null> {
    await this.whenReady();
    let axis = subId.axis;
    let level = subId.level != null ? subId.level : 0;
  ...
  async findLabel<T>(itemLocator: labelSubId): Promise<SlotProxy> {
    await this.whenReady();
    let axis = itemLocator.axis;
    let level = itemLocator.level != null ? itemLocator.level : 0;
    let headerLabel = null;
    headerLabel = await this.getDriver().executeAsyncScript<WebElement>(
      `
      var element = arguments[0];
      var axis = arguments[1];
