...
export class OjStreamList extends OjStreamListBase {
  // Put overrides here
  async findItem<T>(itemLocator: { key?: T; parentKey?: T }): Promise<WebElement> {
    const items = await this.findElements(By.css('.oj-stream-list-item, .oj-stream-list-group'));
    await this.whenBusyContextReady();
    let item = await this.getDriver().executeAsyncScript<WebElement>(
      `
    let el;
  ...
  async findGroup<T>(itemLocator: { key?: T }): Promise<WebElement> {
    const items = await this.findElements(By.css('.oj-stream-list-group'));
    await this.whenBusyContextReady();

    let group = await this.getDriver().executeAsyncScript<WebElement>(
      `
    let el;
    let index;
    var element = arguments[0];
